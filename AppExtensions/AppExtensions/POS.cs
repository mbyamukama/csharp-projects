...
namespace AppExtensions.POS
{
	...
	public static class Code128Rendering
    {
        ...
        public static Image MakeBarcodeImage(string InputData, int BarWeight, bool AddQuietZone, string descr)
        {
            ...
            Bitmap myImg = new Bitmap(myimg);
            int footerHeight = 40;
            if (descr == "") footerHeight = 20;
            Bitmap final = new Bitmap(myimg.Width, myimg.Height + footerHeight);

            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    final.SetPixel(x, y, myImg.GetPixel(x, y));
                }
            }
            myimg.Dispose();

            Graphics gr1 = Graphics.FromImage(final);
            gr1.TextRenderingHint = System.Drawing.Text.TextRenderingHint.SingleBitPerPixel;
            gr1.DrawString(InputData, new Font("Segoe UI", 10, FontStyle.Regular, GraphicsUnit.Point), Brushes.Black, 10, height + 5);

            if (descr != "")
                gr1.DrawString(descr, new Font("Segoe UI", 10, FontStyle.Regular), Brushes.Black, 10, height + 20);
    ...
    public static class EPOSDisplay
    {
        ...
        public static void ShowWelcomeMessage()
        {
            ...
            {
                Clear();
                WriteToUpperLine("WELCOME TO JOMA");
                WriteToLowerLine("SUPERMARKET");
            }
            catch (Exception ex)
            {
                //  MessageBox.Show("An error occurred on the display.\nDETAILS: " + ex.Message);
            }
        }

        public static void WriteToUpperLine(string text)
        {
            try
            {
                byte[] cmd = new byte[] { 0x1B, 0x46, 0x41 };
                byte[] data = ASCIIEncoding.UTF8.GetBytes(text + Environment.NewLine);
                sp.Write(cmd, 0, cmd.Length);
        ...
        public static void WriteToLowerLine(string text)
        {
            try
            {
                byte[] cmd = new byte[] { 0x1B, 0x46, 0x42 };
                byte[] data = ASCIIEncoding.UTF8.GetBytes(text + Environment.NewLine);
                sp.Write(cmd, 0, cmd.Length);
                sp.Write(data, 0, data.Length);
            }
